...
void va_darken(union Pixel* first, union Pixel* last, int darkness)
{
  ...
  for (; first < last-7; first += 8) {
    ...
    __m256i rb = _mm256_and_si256(mask_rb, p);
    rb = _mm256_mullo_epi16(mult_rb, rb);
    rb = _mm256_srli_epi16(rb, 8);
    __m256i ga = _mm256_srli_epi16(p, 8);
    ga = _mm256_mullo_epi16(mult_ga, ga);
    ga = _mm256_andnot_si256(mask_rb, ga);
    p = _mm256_or_si256(rb, ga);
    _mm256_storeu_si256((__m256i*)first, p);
  }
