...
void va_darken(union Pixel* first, union Pixel* last, int darkness)
{
  ...
  for (; first < last-7; first += 8) {
    ...
    __m256i rb = _mm256_srli_epi16(_mm256_mullo_epi16(mult_rb, _mm256_and_si256(mask_rb, p)), 8);
    __m256i ga = _mm256_andnot_si256(mask_rb, _mm256_mullo_epi16(mult_ga, _mm256_srli_epi16(p, 8)));
